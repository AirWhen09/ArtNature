...
$count = "SELECT count(*) as task from task_progress_history where task_id = '$orderNo'";
$getCount = $conn->query($count)->fetch_assoc();
for($i = 0; $i <= 17; ++$i){
    if($i < $getCount['task'] ){
        $progress = $conn->query("SELECT progress from task_progress_history where task_id = '$orderNo' order by date_created limit $i,1")->fetch_assoc();
